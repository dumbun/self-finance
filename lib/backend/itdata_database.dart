...
class BackEndDrift {
  ...
  static Future<int> updateTransactionAsPaid({
    ...
    return (d.update(
      ...
      TransactionsTableCompanion(
        transactionType: Value('Inactive'),
  ...
  static Future<Map<String, int>> deleteTransaction({
    ...
    try {
      return await d.transaction(() async {
        ...
        // Delete related history (your old comment said it, but the sqflite code
        // didn't do it. Keeping it correct here).
  ...
  static Stream<Map<String, num>> watchAnalyticsData() {
    return Stream.fromFuture(db()).asyncExpand((d) {
      final base = d
        ...
        ''',
            variables: [Variable<String>('Active'), Variable<String>('Active')],
  ...
  static Stream<List<Map<String, dynamic>>> watchMonthlyChartData() {
    return Stream.fromFuture(db()).asyncExpand((d) {
      ...
      return _combineLatest2(txns, pays, (_, __) => 0).asyncMap((_) async {
    ...
    final rows = await d
      ...
      SELECT
        ...
        (SELECT COALESCE(SUM(Amount_Paid), 0)
           FROM Payments)  AS paymentsReceived
      ''',
          variables: [Variable<String>('Active'), Variable<String>('Inactive')],
        )
        .get();
