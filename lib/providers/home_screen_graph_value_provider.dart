...
final homeScreenGraphValuesProvider =
    FutureProvider.autoDispose<Map<String, double>>((ref) async {
      final double totalActiveTakenAmount = await ref
          .watch(asyncTransactionsProvider.notifier)
          .fetchSumOfTakenAmount();
      final double totalIntrestAmount = await ref
      ...
      if (totalIntrestAmount != 0.0 || totalActiveTakenAmount != 0.0) {
        ...
        final double firstIndicatorPercentage =
            (totalActiveTakenAmount / totolAmount) * 100;
        final double secoundIndicatorPercentage =
            (totalIntrestAmount / totolAmount) * 100;
        return {
          Constant.firstIndicatorPercentage: Utility.reduceDecimals(
            firstIndicatorPercentage,
          ),
          Constant.secoundIndicatorPercentage: Utility.reduceDecimals(
            secoundIndicatorPercentage,
          ),
        };
      } else {
        return {};
